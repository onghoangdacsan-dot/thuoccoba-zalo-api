...
button.secondary{background:#666}button.ok{background:#16a34a}
...
<thead><tr><th>Mã đơn</th><th>Thời gian</th><th>Khách</th><th>Tổng</th><th>Trạng thái</th><th>Đổi</th></tr></thead>
...
const c={pending:0,preparing:0,shipping:0,completed:0};list.forEach(o=>{if(c[o.status]!=null)c[o.status]++});
...
const s=o.shippingInfo||{};const st=o.status||"pending";
const total=Number(o.total||0).toLocaleString("vi-VN");
const time=o.createdAt?new Date(o.createdAt).toLocaleString("vi-VN"):"—";
return "<tr><td><b>"+o.id+"</b></td><td>"+time+"</td><td>"+(s.fullName||"—")+"<br><span class=muted>"+(s.phone||"")+"</span><br><span class=muted>"+(s.address||"")+"</span></td><td><b>"+total+" đ</b></td><td><span class=\\"badge "+st+"\\">"+(STATUS_LABEL[st]||st)+"</span></td><td class=actions><button onclick=\\"setStatus('"+o.id+"','preparing')\\">Chuẩn bị</button> <button onclick=\\"setStatus('"+o.id+"','shipping')\\">Đang giao</button> <button class=ok onclick=\\"setStatus('"+o.id+"','completed')\\">Đã giao</button></td></tr>"
}).join("")
}
async function setStatus(orderId,status){
const pwd=getPwd();if(!pwd){logout();return}
try{
const res=await fetch("/api/orders/"+encodeURIComponent(orderId)+"/status",{method:"PATCH",headers:{"Content-Type":"application/json","x-admin-password":pwd},body:JSON.stringify({status})});
