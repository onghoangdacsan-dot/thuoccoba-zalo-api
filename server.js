...
// Alias cho Mini App (nếu frontend gọi /api/zalo-phone)
...
// Khách hủy đơn (trong 2 giờ + status pending)
app.post("/api/orders/:orderId/cancel", (req, res) => {
  try {
    ...
    // Chỉ chủ đơn (theo SĐT) mới hủy được
    if (phone && order.shippingInfo?.phone) {
      const a = String(phone).replace(/\D/g, "");
      const b = String(order.shippingInfo.phone).replace(/\D/g, "");
      if (a && b && a !== b) {
    ...
    if (order.status !== "pending") {
      return res.status(400).json({
        error: "Chỉ hủy được đơn đang chờ xác nhận",
      });
    }
    ...
    if (Number.isNaN(created) || Date.now() - created > TWO_HOURS_MS) {
      return res.status(400).json({
        error: "Đã quá 2 giờ — không thể hủy đơn. Vui lòng liên hệ shop.",
      });
    }
