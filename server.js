...
<style>
:root{
  --bg:#0f0c0a; --panel:#1a1512; --card:#231c18; --line:#3d322b;
  --gold:#d4a017; --gold2:#f0c14b; --brown:#8B4513; --text:#f5efe6; --muted:#a89a8c;
...
.sidebar{background:linear-gradient(180deg,#1a1512 0%,#120e0c 100%);border-right:1px solid var(--line);padding:20px 14px;position:sticky;top:0;height:100vh}
...
.brand h1{font-size:14px;margin:0;line-height:1.25}.brand span{font-size:11px;color:var(--muted)}
.nav button{width:100%;text-align:left;background:transparent;color:var(--muted);margin-bottom:4px;display:flex;gap:10px;align-items:center}
...
.topbar h2{margin:0;font-size:22px;font-weight:800;letter-spacing:-.02em}
...
.hero{
  ...
  background:linear-gradient(120deg,#3b2416 0%,#1a1512 45%,#2a1f12 100%);
  border:1px solid var(--line);min-height:140px;padding:22px 24px;
  display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;
}
.hero::before{content:"";position:absolute;inset:0;background:radial-gradient(circle at 80% 20%,rgba(240,193,75,.18),transparent 45%);pointer-events:none}
.hero h3{margin:0 0 6px;font-size:20px;position:relative}
.hero p{margin:0;color:var(--muted);font-size:13px;max-width:520px;position:relative;line-height:1.5}
.hero-chip{position:relative;background:rgba(240,193,75,.15);border:1px solid rgba(240,193,75,.35);color:var(--gold2);padding:6px 12px;border-radius:999px;font-size:12px;font-weight:700}
...
.kpi .value{font-size:22px;font-weight:800;margin-top:6px;color:var(--gold2)}
...
.panel-title{font-size:14px;font-weight:700;margin:0 0 12px;display:flex;align-items:center;gap:8px}
...
input::placeholder{color:#6f645a}
table{width:100%;border-collapse:collapse;font-size:13px}
th{text-align:left;color:var(--muted);font-weight:600;padding:10px 8px;border-bottom:1px solid var(--line);font-size:11px;text-transform:uppercase;letter-spacing:.04em}
td{padding:12px 8px;border-bottom:1px solid rgba(61,50,43,.6);vertical-align:top}
...
.b-ok{background:#14532d;color:#86efac}.b-bad{background:#7f1d1d;color:#fecaca}
.b-jnt{background:#9a3412;color:#fdba74}
...
function applyFilters(){
  ...
  tbody.innerHTML=list.map(o=>{
    ...
    const note=o.note?'<div class="products" style="color:var(--gold2)">Ghi chú: '+escapeHtml(o.note)+"</div>":"";
    const cancel=st==="cancelled"&&o.cancelReason?'<div class="products" style="color:#f87171">Hủy: '+escapeHtml(o.cancelReason)+"</div>":"";
