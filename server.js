...
// Cấu hình kết nối PostgreSQL từ biến môi trường DATABASE_URL
const pool = new Pool({
  connectionString: process.env.DATABASE_URL,
  ssl: { rejectUnauthorized: false }
});

// Khởi tạo bảng lưu đơn hàng trong Database nếu chưa có
async function initDB() {
  try {
    ...
    `);
    console.log("Database PostgreSQL đã sẵn sàng và bảng orders đã được khởi tạo.");
  } catch (err) {
    console.error("Lỗi khởi tạo bảng database:", err);
  }
}
initDB();
...
app.post("/api/get-phone-number", async (req, res) => {
  try {
    ...
    if (!APP_SECRET_KEY) {
      return res.status(500).json({ error: "Chưa cấu hình ZALO_APP_SECRET_KEY trên server" });
    }
...
app.post("/api/zalo-phone", async (req, res) => {
  req.url = "/api/get-phone-number";
  return app._router.handle(req, res, () => {});
});

// Tạo đơn hàng mới (Truyền object trực tiếp vào PostgreSQL JSONB)
...
// Khách hủy đơn (có chuẩn hóa số điện thoại)
app.post("/api/orders/:orderId/cancel", async (req, res) => {
  try {
    ...
    if (Number.isNaN(created) || Date.now() - created > TWO_HOURS_MS) {
      return res.status(400).json({ error: "Đã quá 2 giờ — không thể hủy đơn. Vui lòng liên hệ shop." });
    }
    ...
    console.log("Hủy đơn:", orderId, "→", order.cancelReason);
    res.json(order);
  } catch (err) {
...
app.post("/api/zalo-notify", async (req, res) => {
  try {
    ...
    try {
      ...
      console.error("Không xử lý được extradata:", parseErr);
    }
...
button.secondary{background:#666}button.ok{background:#16a34a}button.jnt{background:#ea580c}button.danger{background:#dc2626}
...
function render(list){
  ...
  document.getElementById("stats").textContent="Tổng "+list.length+" · Chờ "+c.pending+" · Chuẩn bị "+c.preparing+" · Giao "+c.shipping+" · Xong "+c.completed+" · Hủy "+c.cancelled;
  ...
  tbody.innerHTML=list.map(o=>{
    ...
    const cancelNote=st==="cancelled"&&o.cancelReason?'<div class="products" style="color:#991b1b">Lý do: '+o.cancelReason+"</div>":"";
    return '<tr>'+
      '<td><b>'+o.id+'</b></td>'+
      '<td>'+time+'</td>'+
      '<td>'+(s.fullName||"—")+'<br/><span class="muted">'+(s.phone||"")+'</span><br/><span class="muted">'+(s.address||"")+'</span>'+productsHtml+cancelNote+'</td>'+
      '<td><b>'+total+' đ</b></td>'+
...
async function setStatus(orderId,status){
  ...
  try{
    const body={status};
    if(status==="cancelled"){
      const reason=prompt("Lý do hủy (tùy chọn):","Hủy bởi admin")||"Hủy bởi admin";
      body.reason=reason;
    }
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  body { font-family: Arial, Helvetica, sans-serif; width: 100mm; min-height: 150mm; padding: 0; color: #000; background: #fff; }
...
<table class="main">
  <tr class="header">
    <td style="width:38%">Mini App<br/>Thuộc Cô Ba Store</td>
    <td style="width:38%;color:#e11d48">J&amp;T EXPRESS</td>
    <td style="width:24%">ET</td>
  </tr>
  <tr>
...
<script>
  try {
    JsBarcode("#barcode", "\${barcodeValue}", { format: "CODE128", width: 1.4, height: 42, displayValue: false, margin: 0 });
  } catch(e) {}
  try {
    QRCode.toCanvas(document.getElementById("qrcode"), "\${orderCode}", { width: 90, margin: 0 });
  } catch(e) {}
...
app.get("/", (req, res) => {
  res.send("Thuộc Cô Ba Zalo API đang chạy (PostgreSQL Neon). Admin: /admin");
});
