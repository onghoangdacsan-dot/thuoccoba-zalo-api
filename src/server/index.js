...
// Phục vụ giao diện trang admin trực tiếp với đầy đủ tính năng và nút In phiếu J&T
...
// ==========================================
// CÁC API QUẢN LÝ ĐƠN HÀNG (lưu ra file JSON, KHÔNG mất dữ liệu khi restart)
// ==========================================

// File lưu đơn hàng — nằm cạnh file server này
const DATA_DIR = path.join(__dirname, "data");
const ORDERS_FILE = path.join(DATA_DIR, "orders.json");

// Đảm bảo thư mục + file tồn tại trước khi server chạy
...
function readOrders() {
  try {
    ...
    console.error("Lỗi đọc file đơn hàng, trả về danh sách rỗng:", err.message);
    return [];
  }
}

// Ghi toàn bộ đơn hàng xuống file (ghi an toàn: ghi ra file tạm rồi đổi tên)
...
// Lấy danh sách đơn hàng (Mini App / trang admin gọi)
app.get("/api/orders", (req, res) => {
  try {
    const list = readOrders().sort(
...
// Mật khẩu admin lấy từ biến môi trường (KHÔNG hardcode trong code nữa)
// Nhớ tạo file .env với dòng: ADMIN_PASSWORD=mat_khau_cua_ban
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD;
if (!ADMIN_PASSWORD) {
  console.warn("[CẢNH BÁO] Chưa cấu hình ADMIN_PASSWORD trong file .env — mọi yêu cầu đổi trạng thái đơn sẽ bị từ chối.");
}
